...
namespace Lab8.Services
{
    ...
    {
        public SymplexTableModel GetTableModel(string variableSign)
        {
            var a = new double[A.GetLength(0)][];
            for(int i = 0; i < a.Length; i++)
            {
                a[i] = new double[A.GetLength(1)];
                for (int j = 0; j < a[i].Length; j++)
                    a[i][j] = A[i, j];
            }

            var basisNames = new string[B.Length];
            for (int i = 0; i < Basis.Length; i++)
                basisNames[i] = Basis[i] == A.GetLength(1) ? "F" : variableSign + Basis[i].ToString();

            var markingRelations = new double?[MarkingRelations.Length];
            if (!IsOptimalPlan())
                markingRelations = MarkingRelations.
                    Select(m => m <= 0.001 ? 
                    Double.PositiveInfinity : (double?)Math.Round(m, 2)).ToArray();
            markingRelations[markingRelations.Length - 1] = null;

            return new SymplexTableModel
            {
                Coefs = a,
                FreeMembers = B.Select(b => Math.Round(b, 2)).ToArray(),
                MeaurementRelations = markingRelations,

                BasisNames = basisNames,
                VariableNames = Enumerable.Range(0, A.GetLength(1)).
                    Select(i => variableSign + i.ToString()).ToArray(),
                MainColIdx = IsOptimalPlan() ? -1 : MainCol,
                MainRowIdx = IsOptimalPlan() ? -1 : MainRow
            };
        }

        public static SymplexTable GetFromCanonicalForm(CanonicalFormSymplexInput canonicalForm)
        {
            var table = new SymplexTable()
            {
                ...
                Basis = Enumerable.Range(
                        canonicalForm.Matrix.GetLength(1) - canonicalForm.Matrix.GetLength(0) + 1,
